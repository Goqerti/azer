...
/**
 * Yalnız 'Incoming' şöbəsi üçün giriş (finance olmayan rollar).
 * Uğurlu girişdən sonra /index.html səhifəsinə yönləndirir.
 */
exports.login = (req, res) => {
    ...
    // İstifadəçinin mövcudluğunu və finance rolunda OLMADIĞINI yoxlayırıq
    if (user && user.role !== 'finance' && bcrypt.compareSync(password, user.password)) {
        req.session.user = { username, role: user.role, displayName: user.displayName };
        req.session.department = 'incoming'; 
        
        telegram.sendLog(telegram.formatLog(req.session.user, 'sistemə (Incoming) daxil oldu.'));
        res.redirect('/index.html'); // Həmişə əsas panelə yönləndirir
    } else {
        res.redirect('/incoming_login.html?error=true');
    }
};

/**
 * Yalnız 'Finance' şöbəsi üçün giriş.
 * Uğurlu girişdən sonra /finance.html səhifəsinə yönləndirir.
 */
exports.loginFinance = (req, res) => {
    const { username, password } = req.body;
    const users = fileStore.getUsers();
    const user = users[username];

    // İstifadəçinin mövcudluğunu və finance rolunda OLDUĞUNU yoxlayırıq
    if (user && user.role === 'finance' && bcrypt.compareSync(password, user.password)) {
        req.session.user = { username, role: user.role, displayName: user.displayName };
        req.session.department = 'finance';
        telegram.sendLog(telegram.formatLog(req.session.user, 'sistemə (Finance) daxil oldu.'));
        res.redirect('/finance.html');
    } else {
        res.redirect('/finance_login.html?error=true');
    }
};

/**
 * Sistemdən çıxış funksiyası.
 * Sessiyanı məhv edir və əsas şöbə seçimi səhifəsinə yönləndirir.
 */
exports.logout = (req, res) => {
    ...
    req.session.destroy(err => {
        if (err) {
            // Xəta baş verərsə, sadəcə əsas səhifəyə yönləndir
            return res.redirect('/login.html');
        }
        res.clearCookie('connect.sid');
        res.redirect('/login.html'); // Əsas seçim səhifəsinə yönləndirir
    });
};

exports.getCurrentUser = (req, res) => {
    if (req.session.user && req.session.department === 'incoming') {
